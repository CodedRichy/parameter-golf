...
class Hyperparameters:
    ...
    trigram_hash_buckets = int(os.environ.get('TRIGRAM_HASH_BUCKETS', 4095))
...
def eval_val_ttt(args: Hyperparameters, base_model: nn.Module, rank: int, world_size: int, device: torch.device, val_tokens: Tensor, base_bytes_lut: Tensor, has_leading_space_lut: Tensor, is_boundary_token_lut: Tensor) -> tuple[float, float]:
    ...
    lora_params = inject_lora(base_model, rank=args.ttt_lora_rank, device=device)
    val_loss_sum = torch.zeros((), device=device, dtype=torch.float64)
    val_token_count = torch.zeros((), device=device, dtype=torch.float64)
    val_byte_count = torch.zeros((), device=device, dtype=torch.float64)
    base_model.eval()
    for seq_idx in range(seq_start, seq_end):
        ...
        ttt_opt = torch.optim.SGD(lora_params, lr=args.ttt_lr)
        start = seq_idx * seq_len
        tokens = val_tokens[start:start + seq_len + 1].to(device=device, dtype=torch.int64)
        x_full = tokens[:-1].unsqueeze(0)
        y_full = tokens[1:].unsqueeze(0)
        for pos in range(seq_len):
            ttt_opt.zero_grad()
            with torch.autocast(device_type='cuda', dtype=torch.bfloat16, enabled=True):
                per_token_loss = base_model(x_full, y_full, reduction='none')
            pos_loss = per_token_loss[0, pos]
            val_loss_sum += pos_loss.detach().to(torch.float64)
            val_token_count += 1
            tgt_id = y_full[0, pos]
            prev_id = x_full[0, pos]
            tbytes = base_bytes_lut[tgt_id].to(torch.float64)
            tbytes = tbytes + (has_leading_space_lut[tgt_id] & ~is_boundary_token_lut[prev_id]).to(torch.float64)
    ...
    for name, p in base_model.named_parameters():
        p.requires_grad_(orig_requires_grad.get(name, True))
    if dist.is_available() and dist.is_initialized():
        dist.all_reduce(val_loss_sum, op=dist.ReduceOp.SUM)
        dist.all_reduce(val_token_count, op=dist.ReduceOp.SUM)
        dist.all_reduce(val_byte_count, op=dist.ReduceOp.SUM)
...
class CastedLinear(nn.Linear):
    def forward(self, x: Tensor) -> Tensor:
        ...
        if lora_A is not None and lora_B is not None:
            out = out + F.linear(F.linear(x, lora_A.to(x.dtype)), lora_B.to(x.dtype))
        return out
def inject_lora(model: nn.Module, rank: int, device: torch.device) -> list[Tensor]:
...
def reset_lora(model: nn.Module) -> None:
    with torch.no_grad():
        for module in model.modules():
            if isinstance(module, CastedLinear) and getattr(module, 'lora_A', None) is not None:
...
class TrigramHashEmbedding(nn.Module):
    def __init__(self, vocab_size: int, model_dim: int, hash_buckets: int=4095, hash_dim: int=128):
        super().__init__()
        self.base_emb = nn.Embedding(vocab_size, model_dim)
        self.hash_emb = nn.Embedding(hash_buckets, hash_dim)
        self.hash_proj = CastedLinear(hash_dim, model_dim, bias=False)
        self.hash_buckets = hash_buckets
    @property
    def weight(self) -> Tensor:
        return self.base_emb.weight
    def forward(self, input_ids: Tensor) -> Tensor:
        x = self.base_emb(input_ids)
        t_i = input_ids
        t_im1 = F.pad(input_ids[:, :-1], (1, 0), value=0)
        t_im2 = F.pad(input_ids[:, :-2], (2, 0), value=0)
        hash_vals = 36313 * t_i ^ 27191 * t_im1 ^ 51497 * t_im2
        hash_vals = hash_vals % self.hash_buckets
        x = x + self.hash_proj(self.hash_emb(hash_vals))
        return x
class CausalSelfAttention(nn.Module):
    def __init__(self, dim: int, num_heads: int, num_kv_heads: int, rope_base: float, qk_gain_init: float, layer_idx: int=0):
        super().__init__()
        if dim % num_heads != 0:
        ...
        if layer_idx > 0:
            self.v_res_alpha = nn.Parameter(torch.zeros(1, dtype=torch.float32))
            self.v_res_beta = nn.Parameter(torch.ones(1, dtype=torch.float32))
    def forward(self, x: Tensor, v0: Tensor | None=None) -> tuple[Tensor, Tensor]:
        ...
        if self.layer_idx == 0:
            ...
            v_out = None
            if v0 is not None:
                alpha = self.v_res_alpha.to(dtype=v.dtype)
                beta = self.v_res_beta.to(dtype=v.dtype)
                v = alpha * v0 + beta * v
...
class Block(nn.Module):
    def __init__(self, dim: int, num_heads: int, num_kv_heads: int, mlp_mult: int, rope_base: float, qk_gain_init: float, layer_idx: int=0):
        super().__init__()
    ...
    def forward(self, x: Tensor, x0: Tensor, v0: Tensor | None=None) -> tuple[Tensor, Tensor | None]:
        ...
        attn_out, v_cached = self.attn(self.attn_norm(x), v0=v0)
        x = x + self.attn_scale.to(dtype=x.dtype)[None, None, :] * attn_out
        x = x + self.mlp_scale.to(dtype=x.dtype)[None, None, :] * self.mlp(self.mlp_norm(x))
        return (x, v_cached)
class GPT(nn.Module):
    def __init__(self, vocab_size: int, num_layers: int, model_dim: int, num_heads: int, num_kv_heads: int, mlp_mult: int, tie_embeddings: bool, tied_embed_init_std: float, logit_softcap: float, rope_base: float, qk_gain_init: float, trigram_hash_buckets: int=4095, trigram_hash_dim: int=128):
        super().__init__()
        ...
        self.tok_emb = TrigramHashEmbedding(vocab_size, model_dim, trigram_hash_buckets, trigram_hash_dim)
    ...
    def forward(self, input_ids: Tensor, target_ids: Tensor, reduction: str='mean') -> Tensor:
        ...
        for i in range(self.num_encoder_layers):
            x, v_cached = self.blocks[i](x, x0, v0=v0)
            if i == 0:
                v0 = v_cached
            skips.append(x)
        ...
        if reduction == 'none':
            return loss.view(input_ids.shape[0], input_ids.shape[1])
        return loss
def main() -> None:
    ...
    base_model = GPT(vocab_size=args.vocab_size, num_layers=args.num_layers, model_dim=args.model_dim, num_heads=args.num_heads, num_kv_heads=args.num_kv_heads, mlp_mult=args.mlp_mult, tie_embeddings=args.tie_embeddings, tied_embed_init_std=args.tied_embed_init_std, logit_softcap=args.logit_softcap, rope_base=args.rope_base, qk_gain_init=args.qk_gain_init, trigram_hash_buckets=args.trigram_hash_buckets, trigram_hash_dim=args.trigram_hash_dim).to(device).bfloat16()
    ...
    optimizer_tok = torch.optim.AdamW([{'params': list(base_model.tok_emb.parameters()), 'lr': token_lr, 'base_lr': token_lr}], betas=(args.beta1, args.beta2), eps=args.adam_eps, weight_decay=args.weight_decay, fused=True)
    optimizer_muon = Muon(matrix_params, lr=args.matrix_lr, momentum=args.muon_momentum, backend_steps=args.muon_backend_steps, weight_decay=args.weight_decay)
    ...
    if base_model.lm_head is not None:
        optimizer_head = torch.optim.AdamW([{'params': [base_model.lm_head.weight], 'lr': args.head_lr, 'base_lr': args.head_lr}], betas=(args.beta1, args.beta2), eps=args.adam_eps, weight_decay=args.weight_decay, fused=True)
    ...
    if args.ttt_enabled:
        ...
        ttt_val_loss, ttt_val_bpb = eval_val_ttt(args, base_model, rank, world_size, device, val_tokens, base_bytes_lut, has_leading_space_lut, is_boundary_token_lut)
        torch.cuda.synchronize()
        log0(f'ttt_eval val_loss:{ttt_val_loss:.4f} val_bpb:{ttt_val_bpb:.4f} eval_time:{1000.0 * (time.perf_counter() - t_ttt):.0f}ms lora_rank:{args.ttt_lora_rank} ttt_lr:{args.ttt_lr}')
